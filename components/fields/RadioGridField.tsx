...
const type = "RadioGridField" as ElementsType;
...
export const RadioGridFieldFormElement: FormElement = {
  type,
  ...
  validate: (
    ...
    if (element.extraAttributes.required) {
      ...
      try {
        ...
        return Object.values(parsed).some((val: any) => val !== null && val !== "");
      } catch {
        return false;
      }
    }
...
const RadioIcon = ({ checked, error, disabled }: { checked: boolean; error?: boolean; disabled?: boolean }) => (
  <div className={cn(
                ...
                {columns.map((_, cIdx) => (
                  <td key={cIdx} className="p-2 text-center pointer-events-none">
                    <div className="flex items-center justify-center">
                      <RadioIcon checked={false} disabled={isSelected} />
                    </div>
                  </td>
                ))}
                {isSelected && <td></td>}
...
function FormComponent({
  ...
  // State is a record: { [rowIndex]: colIndexValue }
  // e.g. { "Row 1": "Column 1" }
  ...
  const setSelection = (row: string, col: string) => {
    const newValues = { ...value, [row]: col };
    setValue(newValues);
    if (submitValue) {
      const stringified = JSON.stringify(newValues);
      const valid = RadioGridFieldFormElement.validate(element, stringified);
      setError(!valid);
                ...
                {columns.map((col, cIdx) => {
                  ...
                  return (
                    <td key={cIdx} className="p-3 text-center">
                      ...
                      >
                        <RadioIcon checked={isChecked} error={error} />
                      </div>
                    </td>
                  );
                })}
